...
import type { PayFrequency, PayrollAuditEntry, PayrollCalculationSnapshot, PayrollPeriod, PayrollRecord, PayrollStatus, PayrollValidationReport } from '../../../types'
...
const payrollPermissionsByRole: Record<string, string[] | '*'> = {
  admin: '*',
  super_admin: '*',
  payroll_preparer: ['create_period', 'process', 'validate', 'reprocess', 'view'],
  payroll_approver: ['validate', 'approve', 'request_correction', 'view'],
  payroll_releaser: ['release', 'view'],
  auditor: ['view', 'view_audit_logs'],
}
...
export default function PayrollPage() {
  ...
  const renderActions = (period: PayrollPeriod) => {
    ...
    const approveBlocked = Boolean(
      ...
      (validationReport?.criticalIssueCount ?? 0) > 0
    )
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <StatCard
          label="Validation"
          value={validationReport?.periodId === focusPeriod?.id ? validationReport?.criticalIssueCount ?? 0 : focusPeriod?.warningCount ?? 0}
          delta={validationReport?.periodId === focusPeriod?.id
            ? (validationReport?.isValid ? 'Validation Passed' : 'Fix Issues Before Approval')
